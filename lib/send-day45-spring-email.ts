...
const SPRING_URL = "https://springfinancial.ca/foundation/";

export type Day45SpringEmailResult =
  | { sent: true }
...
export async function sendDay45SpringEmail(to: string, name: string, unsubscribeUrl: string): Promise<Day45SpringEmailResult> {
  ...
  const subject = firstName
    ? `${firstName}, build credit and savings at the same time`
...
<div style="font-family:'Segoe UI','Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;color:#0F1923;">
  ...
  <div style="background:#0F1923;padding:32px 40px;border-radius:16px 16px 0 0;">
    ...
    <h1 style="margin:0;font-size:26px;font-weight:800;color:#ffffff;letter-spacing:-0.02em;line-height:1.2;">Two wins.<br>One move.</h1>
    <p style="margin:10px 0 0;font-size:13px;color:rgba(255,255,255,0.5);font-weight:500;">A way to build your credit and your savings together.</p>
  </div>

  <!-- BODY -->
  <div style="padding:32px 40px;">
    <p style="font-size:15px;font-weight:600;color:#0F1923;margin:0 0 6px;">Hi ${escapeHtml(firstName || name)},</p>
    <p style="font-size:14px;line-height:1.8;color:#4B5563;margin:0 0 18px;">Here's something most people don't know exists: a way to build your credit history and put money aside at the same time — without taking on debt.</p>
    <p style="font-size:14px;line-height:1.8;color:#4B5563;margin:0 0 18px;">It's called a secured savings loan from Spring Financial. Here's how it works: instead of borrowing cash upfront, you make small regular payments that are reported to the credit bureaus — building your payment history — and at the end, <strong style="color:#0F1923;">you get the money back as savings.</strong> You're not taking on cash debt. You're building credit and a cushion at the same time.</p>
    <p style="font-size:14px;line-height:1.8;color:#4B5563;margin:0 0 28px;">It's one of the safest ways to add positive history to your file while you work through your monthly actions.</p>

    <!-- CTA -->
    <div style="text-align:center;margin:0 0 32px;">
      <a href="${SPRING_URL}" style="display:inline-block;background:#00C9A7;color:#0F1923;padding:14px 36px;border-radius:100px;text-decoration:none;font-weight:800;font-size:13px;letter-spacing:0.06em;text-transform:uppercase;">See How It Works</a>
    </div>
