...
When recommending a secured credit card in **top_actions**, the **action** string must be concise and use **markdown** inline links on the card names (required for the client UI). Use this **exact** string (including the leading **🏦** emoji and single space per TOP_ACTIONS EMOJI PREFIX):
"🏦 Add a secured credit card — [Neo Financial](https://neo.cc/refer/G3Y6L5A9), [Tangerine](https://www.tangerine.ca) (use code 79976711S1 for $50 bonus), or [Koho](https://www.koho.ca) — to build history toward three healthy revolving accounts."
Never use "on the Visa or Mastercard network" or "so you can" phrasing in this action. Links must stay inline with the card names as shown.
