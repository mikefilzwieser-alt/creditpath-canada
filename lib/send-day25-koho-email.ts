...
const KOHO_URL = "https://www.koho.ca/credit-building/";
const REFERRAL_CODE = "68KDVQN2DC";

export type Day25KohoEmailResult =
  | { sent: true }
...
export async function sendDay25KohoEmail(to: string, name: string, unsubscribeUrl: string): Promise<Day25KohoEmailResult> {
  ...
  const subject = firstName
    ? `${firstName}, the one approval that can't say no`
    : "The one approval that can't say no";
...
<div style="font-family:'Segoe UI','Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;color:#0F1923;">
  ...
  <div style="background:#0F1923;padding:32px 40px;border-radius:16px 16px 0 0;">
    ...
    <h1 style="margin:0;font-size:26px;font-weight:800;color:#ffffff;letter-spacing:-0.02em;line-height:1.2;">The one approval<br>that can't say no.</h1>
    <p style="margin:10px 0 0;font-size:13px;color:rgba(255,255,255,0.5);font-weight:500;">Guaranteed. No credit check. Two minutes.</p>
  </div>

  <!-- BODY -->
  <div style="padding:32px 40px;">
    <p style="font-size:15px;font-weight:600;color:#0F1923;margin:0 0 6px;">Hi ${escapeHtml(firstName || name)},</p>
    <p style="font-size:14px;line-height:1.8;color:#4B5563;margin:0 0 18px;">If you've been turned down before, this one's different. KOHO's Secured Credit Building tool offers <strong style="color:#0F1923;">guaranteed approval</strong> — no credit check, no risk of another rejection on your file. Because it's secured, there's nothing to decline.</p>
    <p style="font-size:14px;line-height:1.8;color:#4B5563;margin:0 0 28px;">KOHO reports an average 93-point increase for users on their credit-building tools. <span style="color:#9CA3AF;font-size:11px;">(Per KOHO. Outcomes vary.)</span></p>

    <p style="margin:0 0 14px;text-align:center;font-size:14px;line-height:1.6;color:#0F1923;"><strong style="color:#0F1923;">Guaranteed approval. Zero risk. Two minutes.</strong></p>
    <p style="margin:0 0 28px;text-align:center;font-size:12px;line-height:1.6;color:#6B7A8D;">Use referral code <strong style="color:#0F1923;">${REFERRAL_CODE}</strong> when you apply.</p>

    <!-- CTA -->
    <div style="text-align:center;margin:0 0 32px;">
      <a href="${KOHO_URL}" style="display:inline-block;background:#00C9A7;color:#0F1923;padding:14px 36px;border-radius:100px;text-decoration:none;font-weight:800;font-size:13px;letter-spacing:0.06em;text-transform:uppercase;">See the Guaranteed Option</a>
    </div>
